import { CheckCircle, Link, FileSignature, AlertTriangle, Ban } from "lucide-react";
    ...
    <Card data-testid="card-verification-results">
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Badge 
            variant={results.isRevoked ? "destructive" : results.confidenceScore >= 70 ? "default" : "destructive"}
            className={
              results.isRevoked ? 'bg-red-100 text-red-800' :
              results.confidenceScore >= 70 ? 'bg-green-100 text-green-800' : ''
            }
          >
            {results.isRevoked ? '⛔ Revoked' : results.confidenceScore >= 70 ? 'Verified' : 'Failed'}
          </Badge>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        {results.isRevoked && (
          <div className="bg-red-50 border-2 border-red-300 rounded-lg p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h4 className="font-bold text-red-800 text-lg">⛔ DOCUMENT REVOKED</h4>
                <p className="text-sm text-red-700 mt-1">
                  This document was found on the blockchain but has been revoked by the issuer.
                  It is no longer valid.
                </p>
                {results.matchedBatch?.revokedAt && (
                  <p className="text-xs text-red-600 mt-2">
                    Revoked on: {new Date(results.matchedBatch.revokedAt).toLocaleString()}
                  </p>
                )}
              </div>
            </div>
        ...
        {/* No Match Found */}
        {!results.documentFound && !results.isRevoked && (
