...
export default function ProcessingStatus({ isProcessing, onProcessingComplete }: ProcessingStatusProps) {
  ...
  useEffect(() => {
    ...
    let cancelled = false; // Cleanup flag to prevent state updates on unmounted component

    const processSteps = async () => {
      for (let i = 0; i < steps.length; i++) {
        if (cancelled) return; // Exit early if component unmounted
      ...
      setTimeout(() => {
        if (!cancelled) { // Final check before cleanup
          setIsVisible(false);
          setProgress(0);
    ...
    // Cleanup function to prevent memory leaks (FIX for BUG #6)
    return () => {
      cancelled = true;
    };
  }, [isVisible]); // Removed steps from dependencies - it's constant
